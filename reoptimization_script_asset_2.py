...
class automation(tpqoa_reoptimization.tpqoa): 
    ...
    def __init__(self, # Reoptimization Class 
                 ...
                 symbol = 'XAU_CAD', start = str(dt.datetime.now().date() - dt.timedelta(days = 142)), 
                 ...
                 streaming_list = ['GBP_USD', 'EUR_USD'],
                 price_ask = 'A', price_bid = 'B', averaging_period = 168, no_of_elements = 168, spread = 0.00009, 
                 begining_test_env = 25, instruments_traded = 3, position_sizing_factor_after_VaR = 0.2, 
                 # Trading Class
                 granularity_resam = '30Min', units = 1000, verbose = True, log_file = 'the_project_reoptimization_XAU_CAD.log'):
        super(automation, self).__init__(config_file)
        
        self.log_file = log_file
        self.symbol = symbol
        self.symbol_another = symbol_another
        self.start = start
        self.end = end
        ...
        self.streaming_list = streaming_list
    ...
    def set_socket(self):
        ...
        socket.bind('tcp://127.0.0.1:1965') # IP Address of digitalocean
        self.socket = socket
    ...
    def hyperparameter_tuning(self):
        ...
        self.trading_model = load_model('trading_model_XAU_CAD.h5')
        print('trading_model_XAU_CAD.h5 has been loaded from SSD.')
        self.logger_monitor('trading_model_XAU_CAD.h5 has been loaded from SSD.')
        ...
        for self.hl in self.hidden_layers:
            for self.lr in self.learning_rates:
                ...
                if self.net_leveraged_returns_hyper > self.max_net_leveraged_returns_hyper:
                    ...
                    print(f'New maximum for net leveraged returns after tc of {self.max_net_leveraged_returns_hyper} is achieved with an out-of-sample performance of {self.acc_te:.4f}')
                    self.logger_monitor(f'New maximum for net leveraged returns after tc of {self.max_net_leveraged_returns_hyper} is achieved with an out-of-sample performance of {self.acc_te:.4f}')
        try:
            self.model_persisted.save('reoptimized_model_XAU_CAD.h5')
            print(f'model_persisted is saved to SSD @ {dt.datetime.now()} as reoptimized_model_XAU_CAD.h5')
            self.logger_monitor(f'model_persisted is saved to SSD @ {dt.datetime.now()} as reoptimized_model_XAU_CAD.h5')
        except:
            self.trading_model.save('reoptimized_model_XAU_CAD.h5')
            print(f'No model that has a net return (after tc) of greater than zero is found. The previous model (last month) is used. trading_model_XAU_CAD is saved to SSD as reoptimized_model_XAU_CAD.h5 @ {dt.datetime.now()}')
            self.logger_monitor(f'No model that has a net return (after tc) of greater than zero is found. The previous model (last month) is used. trading_model_XAU_CAD is saved to SSD as reoptimized_model_XAU_CAD.h5 @ {dt.datetime.now()}')
    ...
    def units_optimization_realized_pl(self):
        ...
        self.pkl_file_realized_pl = open('pl_gbp_usd.pkl', 'rb')
        self.pl_realized = pickle.load(self.pkl_file_realized_pl)
        print('pl_gbp_usd.pkl is loaded from SSD & self.pl_realized is initialized.')
        self.logger_monitor('pl_gbp_usd.pkl is loaded from SSD & self.pl_realized is initialized.')
        self.pkl_file_realized_pl.close()
        
        try: # remove units_initial_setup.pkl from drive after 1st reoptimization
            self.pkl_file_units_initial_position = open('units_initial_setup_gbp_usd.pkl', 'rb') # 1000 units
            self.initial_position_sizing_lev = pickle.load(self.pkl_file_units_initial_position)
            print('units_initial_setup_gbp_usd.pkl is loaded from SSD & self.initial_position_sizing_lev is initialized.')
            self.logger_monitor('units_initial_setup_gbp_usd.pkl is loaded from SSD & self.initial_position_sizing_lev is initialized.')
            self.pkl_file_units_initial_position.close()
        except:
            self.pkl_file_units_initial_position = open('units_transfer_gbp_usd.pkl', 'rb')
            self.initial_position_sizing_lev = pickle.load(self.pkl_file_units_initial_position)
            print('units_transfer_gbp_usd.pkl is loaded from SSD & self.initial_position_sizing_lev is initialized.')
            self.logger_monitor('units_transfer_gbp_usd.pkl is loaded from SSD & self.initial_position_sizing_lev is initialized.')
            self.pkl_file_units_initial_position.close()
            ...
            if self.position == 1:
                ...
                if float(df[f'{instrument}_bid'].values) <= self.stopout_price_long:
                    ...
                    if self.optimal_leverage_half_kelly < 20:
                        print('VaR level breached. Close out order executed. Trading suspended for 2 hrs because calculated optimal leverage (half kelly) is below 20.')
                        self.logger_monitor('VaR level breached. Close out order executed. Trading suspended for 2 hrs because calculated optimal leverage (half kelly) is below 20.')
    ...
    def on_success(self, df, instrument):
        ...
        if instrument == self.symbol_another:
            self.eurusd_bid = float(df[f'{instrument}_bid'].values)

        if instrument == self.symbol:
            self.gbpusd_ask = float(df[f'{instrument}_ask'].values)
    ...
    def on_success_heartbeat(self, heartbeat):
       ...
       #     self.eurusd_mid = (self.eurusd_bid + self.eurusd_ask) / 2
        ...
        #if dt.datetime.now().minute == 43: # Manual optimization due to self.units    
            
        if dt.datetime.now().day == 28 and dt.datetime.now().hour == 13 and dt.datetime.now().minute == 5:
            ...
            if self.time_flag_reoptimization:
                ...
                self.pkl_file_df_test = open('df_test_transfer_gbp_usd.pkl', 'wb')
                ...
                self.units = (self.units * self.eurusd_bid) / self.gbpusd_ask # Trading GBP/USD, so units is converted from Euros to GBP. GBP is home currency. Order is sent in as GBP.
                self.pkl_file_units = open('units_transfer_gbp_usd.pkl', 'wb')
                pickle.dump(self.units, self.pkl_file_units)
                print(f'self.units is saved on SSD & {self.units} units will be traded.') 
                ...
                print('\n' + 96 * '=' + '\n')
                self.logger_monitor('\n' + 96 * '=' + '\n')
                self.pkl_file_vAr = open('vAr_transfer_gbp_usd.pkl', 'wb')
                ...
                self.logger_monitor(f'self.vAr is saved on SSD & if market moves AGAINST my strategy by {self.vAr}, tail end (max) loss is incurred.')
                self.pkl_file_vAr.close()
                print('\n' + 96 * '=' + '\n')
                self.logger_monitor('\n' + 96 * '=' + '\n')
                self.time_close = dt.datetime.now() 
                print(f'Reoptimization process completed @ {self.time_close}. Time taken for reoptimization process is {self.time_close - self.time_open}')
                self.logger_monitor(f'Reoptimization process completed @ {self.time_close}. Time taken for reoptimization process is {self.time_close - self.time_open}')
                print('\n' + 96 * '=' + '\n')
                self.logger_monitor('\n' + 96 * '=' + '\n')
...
if __name__ == '__main__':
    ...
    stream_list = str(auto.streaming_list).replace("['", "").replace("', '", ",").replace("']", "")
