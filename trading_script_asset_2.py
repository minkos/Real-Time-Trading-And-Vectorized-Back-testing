...
class automation(tpqoa_trading_gbp_usd.tpqoa): 
    ...
    def __init__(self, model_persisted, vAr, units, df_test,# Trading Class 
                 ...
                 price_ask = 'A', price_bid = 'B', averaging_period = 168, no_of_elements = 168, spread = 0.0001, 
                 ...
                 log_file = 'the_project_trading_XAU_CAD.log'):
    ...
    def set_socket(self):
        ...
        socket.bind('tcp://127.0.0.1:1935') # IP address of digitalocean
        self.socket = socket
    ...
    def report_trade(self, time, side, order):
        ...
        plt.savefig('Performance Chart - (All Trades) - GBP_USD')
        print(80 * '=')
    ...
    def on_success(self, df, instrument):
       ...
       # self.VaR_check(df, instrument)
        ...
        if len(self.data) > self.min_length:
            ...
            if (dt.datetime.today().weekday() in [0, 1, 2, 3, 4] and dt.datetime.now().hour >= 12 and dt.datetime.now().hour <= 23) or\
    ...
    def on_success_heartbeat(self, heartbeat):
        ...
        if dt.datetime.today().weekday() == 5 and dt.datetime.now().hour == 4 and dt.datetime.now().minute == 55:
            if self.master_n_side_kick_frame_save_time_flag:
                self.pkl_file_master_frame = open('master_frame_gbp_usd.pkl', 'wb')
                self.pkl_file_side_kick_frame = open('side_kick_frame_gbp_usd.pkl', 'wb')
                self.pkl_file_resampled_data_audit = open('resampled_data_audit_gbp_usd.pkl', 'wb')
                self.pkl_file_gbp_usd = open('pl_gbp_usd.pkl', 'wb')
                self.pkl_file_gbp_usd_trades = open('trades_gbp_usd.pkl', 'wb')
                ...
                pickle.dump(self.pl, self.pkl_file_gbp_usd)
                pickle.dump(self.trades, self.pkl_file_gbp_usd_trades)
                ...
                self.pkl_file_gbp_usd_trades.close()
        ...
        if dt.datetime.today().weekday() == 0 and dt.datetime.now().hour == 5 and dt.datetime.now().minute == 0:
            if self.master_n_side_kick_frame_load_time_flag:
                ...
                self.reset_frames()
                self.pkl_file_master_frame = open('master_frame_gbp_usd.pkl', 'rb')
                self.pkl_file_side_kick_frame = open('side_kick_frame_gbp_usd.pkl', 'rb')
                self.pkl_file_gbp_usd = open('pl_gbp_usd.pkl', 'rb')
                self.pkl_file_gbp_usd_trades = open('trades_gbp_usd.pkl', 'rb')
                self.master_frame = pickle.load(self.pkl_file_master_frame)
                self.side_kick_frame = pickle.load(self.pkl_file_side_kick_frame)
                self.pl = pickle.load(self.pkl_file_gbp_usd)
                self.trades = pickle.load(self.pkl_file_gbp_usd_trades)
                self.pkl_file_master_frame.close()
                self.pkl_file_side_kick_frame.close()
                self.pkl_file_gbp_usd.close()
                self.pkl_file_gbp_usd_trades.close()
                ...
                self.master_frame = self.master_frame.append({'GBP_USD_bid' : np.nan, 'GBP_USD_ask' : np.nan,
                            'Time' : str(dt.datetime.now())}, ignore_index = True)

                self.side_kick_frame = self.side_kick_frame.append(
                    {'GBP_USD_bid_Lowest_transfer_in_progress' : np.nan, 'GBP_USD_ask_Highest_transfer_in_progress' : np.nan,
                        'Time' : str(dt.datetime.now())}, ignore_index = True)
        ...
        if dt.datetime.now().day == 28 and dt.datetime.now().hour == 12 and dt.datetime.now().minute == 1:
            if self.time_flag_unload:
                print('\n' + 96 * '=' + '\n')
                self.logger_monitor('\n' + 96 * '=' + '\n')
                self.model_persisted.save('trading_model_gbp_usd.h5')
        ...
        if dt.datetime.now().day >= 1 and dt.datetime.now().day <= 8 and dt.datetime.today().weekday() in [0] and dt.datetime.now().hour == 4 and dt.datetime.now().minute == 45:
            ...
            if self.load_reoptimized_model_time_flag:
                ...
                try:
                    ...
                    print('\n' + 96 * '=' + '\n')
                    self.logger_monitor('\n' + 96 * '=' + '\n')
                    self.model_persisted = load_model('reoptimized_model_gbp_usd.h5')
                    ...
                    print('\n' + 96 * '=' + '\n')
                    self.logger_monitor('\n' + 96 * '=' + '\n')
                    self.pkl_file_units = open('units_transfer_gbp_usd.pkl', 'rb')
                    ...
                    print('\n' + 96 * '=' + '\n')
                    self.logger_monitor('\n' + 96 * '=' + '\n')
                    self.pkl_file_vAr = open('vAr_transfer_gbp_usd.pkl', 'rb')
                    ...
                    print('\n' + 96 * '=' + '\n')
                    self.logger_monitor('\n' + 96 * '=' + '\n')
                    self.pkl_file_df_test = open('df_test_transfer_gbp_usd.pkl', 'rb')
                    self.df_test = pickle.load(self.pkl_file_df_test)
                    self.pkl_file_df_test.close()
                    print('df_test_transfer_gbp_usd is loaded from SSD. Use jupyter notebook to check if needed.')
                    self.logger_monitor('df_test_transfer_gbp_usd is loaded from SSD. Use jupyter notebook to check if needed.')
                except:
...
if __name__ == '__main__':
    ...
    model_persisted = load_model('trading_model_gbp_usd.h5')
    print(f'model_persisted is initialized with trading_model_gbp_usd.h5 from SSD @ {dt.datetime.now()}')
    
    pkl_file_vAr_initial_setup = open('vAr_initial_setup_gbp_usd.pkl', 'rb')
    vAr = pickle.load(pkl_file_vAr_initial_setup)
    pkl_file_vAr_initial_setup.close()
    print(f'vAr is initialized with vAr_initial_setup_gbp_usd.pkl from SSD @ {dt.datetime.now()}')
    
    pkl_file_units_initial_setup = open('units_initial_setup_gbp_usd.pkl', 'rb')
    units = pickle.load(pkl_file_units_initial_setup)
    pkl_file_units_initial_setup.close()
    
    pkl_file_df_test = open('df_test_setup_gbp_usd.pkl', 'rb')
