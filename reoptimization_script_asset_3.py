...
class automation(tpqoa_reoptimization.tpqoa): 
    ...
    def __init__(self, # Reoptimization Class 
                 ...
                 # Trading Class
                 granularity_resam = '30Min', units = 1000, verbose = True, log_file = 'the_project_reoptimization_usd_jpy.log'):
    ...
    def set_socket(self):
        ...
        socket.bind('tcp://127.0.0.1:2033') # IP Address of digitalocean
        self.socket = socket
    ...
    def hyperparameter_tuning(self):
        ...
        self.trading_model = load_model('trading_model_XAU_CHF.h5')
        print('trading_model_XAU_CHF.h5 has been loaded from SSD.')
        self.logger_monitor('trading_model_XAU_CHF.h5 has been loaded from SSD.')
        ...
        try:
            self.model_persisted.save('reoptimized_model_XAU_CHF.h5')
            print(f'model_persisted is saved to SSD @ {dt.datetime.now()} as reoptimized_model_XAU_CHF.h5')
            self.logger_monitor(f'model_persisted is saved to SSD @ {dt.datetime.now()} as reoptimized_model_XAU_CHF.h5')
        except:
            self.trading_model.save('reoptimized_model_XAU_CHF.h5')
            print(f'No model that has a net return (after tc) of greater than zero is found. The previous model (last month) is used. trading_model_XAU_CHF is saved to SSD as reoptimized_model__XAU_CHF.h5 @ {dt.datetime.now()}')
            self.logger_monitor(f'No model that has a net return (after tc) of greater than zero is found. The previous model (last month) is used. trading_model_XAU_CHF is saved to SSD as reoptimized_model_XAU_CHF.h5 @ {dt.datetime.now()}')
    ...
    def units_optimization_realized_pl(self):
        ...
        self.pkl_file_realized_pl = open('pl_usd_jpy.pkl', 'rb')
        self.pl_realized = pickle.load(self.pkl_file_realized_pl)
        print('pl_usd_jpy.pkl is loaded from SSD & self.pl_realized is initialized.')
        self.logger_monitor('pl_usd_jpy.pkl is loaded from SSD & self.pl_realized is initialized.')
        self.pkl_file_realized_pl.close()
        
        try: # remove units_initial_setup.pkl from drive after 1st reoptimization
            self.pkl_file_units_initial_position = open('units_initial_setup_usd_jpy.pkl', 'rb') # 1000 units
            self.initial_position_sizing_lev = pickle.load(self.pkl_file_units_initial_position)
            print('units_initial_setup_usd_jpy.pkl is loaded from SSD & self.initial_position_sizing_lev is initialized.')
            self.logger_monitor('units_initial_setup_usd_jpy.pkl is loaded from SSD & self.initial_position_sizing_lev is initialized.')
            self.pkl_file_units_initial_position.close()
        except:
            self.pkl_file_units_initial_position = open('units_transfer_usd_jpy.pkl', 'rb')
            self.initial_position_sizing_lev = pickle.load(self.pkl_file_units_initial_position)
            print('units_transfer_usd_jpy.pkl is loaded from SSD & self.initial_position_sizing_lev is initialized.')
            self.logger_monitor('units_transfer_usd_jpy.pkl is loaded from SSD & self.initial_position_sizing_lev is initialized.')
            self.pkl_file_units_initial_position.close()
    ...
    def on_success_heartbeat(self, heartbeat):
       ...
       #     self.eurusd_mid = (self.eurusd_bid + self.eurusd_ask) / 2
        ...
        #if dt.datetime.now().minute == 58: # Manual optimization due to self.units
            
        if dt.datetime.now().day == 28 and dt.datetime.now().hour == 14 and dt.datetime.now().minute == 5:
            ...
            if self.time_flag_reoptimization:
                ...
                self.pkl_file_df_test = open('df_test_transfer_usd_jpy.pkl', 'wb')
                ...
                self.units = self.units * self.eurusd_bid  # Trading USD/JPY, so units is converted from Euros to USD. USD is home currency. Order is sent in using USD.
                self.pkl_file_units = open('units_transfer_usd_jpy.pkl', 'wb')
                ...
                print('\n' + 96 * '=' + '\n')
                self.logger_monitor('\n' + 96 * '=' + '\n')
                self.pkl_file_vAr = open('vAr_transfer_usd_jpy.pkl', 'wb')
