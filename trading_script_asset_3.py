...
class automation(tpqoa_trading_usd_jpy.tpqoa): 
    ...
    def __init__(self, model_persisted, vAr, units, df_test, # Trading Class 
                 ...
                 price_ask = 'A', price_bid = 'B', averaging_period = 168, no_of_elements = 168, spread = 0.00003, 
                 ...
                 log_file = 'the_project_trading_XAU_CHF.log'):
    ...
    def set_socket(self):
        ...
        socket.bind('tcp://127.0.0.1:1967') # IP address of digitalocean
        self.socket = socket
    ...
    def generate_all_env(self):
        ...
        print(f'Finished data retrival @ {self.retrieve_end}. Time taken is {self.retrieve_end - self.retrieve_start}. All environment is successfully initialized.') 
        self.logger_monitor(f'Finished data retrival @ {self.retrieve_end}. Time taken is {self.retrieve_end - self.retrieve_start}. All environment is successfully initialized.')
        ...
        for self.hl in self.hidden_layers:
            for self.lr in self.learning_rates:
                ...
                if self.acc_te > self.max_acc_te:
                    ...
                    print(f'New max_acc_te is achieved with Hidden Layers: {self.hl} | Learning Rate: {self.lr} ||. Model is persisted.\n')
                    self.logger_monitor(f'New max_acc_te is achieved with Hidden Layers: {self.hl} | Learning Rate: {self.lr} ||. Model is persisted.\n')   
    ...
    def report_trade(self, time, side, order):
        ...
        plt.savefig('Performance Chart - (All Trades) - USD_JPY')
        print(80 * '=')
                ...
                if self.trading_time:
                    if self.position in [-1]:
                        try:
                            ...
                            print('Closing out LONG order did not go through. Most likely market is closed.')
                            self.logger_monitor('Closing out LONG order did not go through. Most likely market is closed.')
                        ...
                        try:
                            ...
                            print('Closing out SHORT order did not go through. Most likely market is closed.')
                            self.logger_monitor('Closing out SHORT order did not go through. Most likely market is closed.')
    ...
    def on_success_heartbeat(self, heartbeat):
        ...
        if dt.datetime.today().weekday() == 5 and dt.datetime.now().hour == 4 and dt.datetime.now().minute == 55:
            if self.master_n_side_kick_frame_save_time_flag:
                self.pkl_file_master_frame = open('master_frame_usd_jpy.pkl', 'wb')
                self.pkl_file_side_kick_frame = open('side_kick_frame_usd_jpy.pkl', 'wb')
                self.pkl_file_resampled_data_audit = open('resampled_data_audit_usd_jpy.pkl', 'wb')
                self.pkl_file_usd_jpy = open('pl_usd_jpy.pkl', 'wb')
                self.pkl_file_usd_jpy_trades = open('trades_usd_jpy.pkl', 'wb')
                ...
                pickle.dump(self.pl, self.pkl_file_usd_jpy)
                pickle.dump(self.trades, self.pkl_file_usd_jpy_trades)
                ...
                self.pkl_file_usd_jpy_trades.close()
        ...
        if dt.datetime.today().weekday() == 0 and dt.datetime.now().hour == 5 and dt.datetime.now().minute == 0:
            if self.master_n_side_kick_frame_load_time_flag:
                ...
                self.reset_frames()
                self.pkl_file_master_frame = open('master_frame_usd_jpy.pkl', 'rb')
                self.pkl_file_side_kick_frame = open('side_kick_frame_usd_jpy.pkl', 'rb')
                self.pkl_file_usd_jpy = open('pl_usd_jpy.pkl', 'rb')
                self.pkl_file_usd_jpy_trades = open('trades_usd_jpy.pkl', 'rb')
                self.master_frame = pickle.load(self.pkl_file_master_frame)
                self.side_kick_frame = pickle.load(self.pkl_file_side_kick_frame)
                self.pl = pickle.load(self.pkl_file_usd_jpy)
                self.trades = pickle.load(self.pkl_file_usd_jpy_trades)
                self.pkl_file_master_frame.close()
                self.pkl_file_side_kick_frame.close()
                self.pkl_file_usd_jpy.close()
                self.pkl_file_usd_jpy_trades.close()
                print(f'self.master_frame, self.side_kick_frame, self.pl and self.trades are loaded from SSD @ {dt.datetime.now()}')
                self.logger_monitor(f'self.master_frame, self.side_kick_frame, self.pl and self.trades are loaded from SSD @ {dt.datetime.now()}')
                
                self.master_frame = self.master_frame.append({'USD_JPY_bid' : np.nan, 'USD_JPY_ask' : np.nan,
                            'Time' : str(dt.datetime.now())}, ignore_index = True)

                self.side_kick_frame = self.side_kick_frame.append(
                    {'USD_JPY_bid_Lowest_transfer_in_progress' : np.nan, 'USD_JPY_ask_Highest_transfer_in_progress' : np.nan,
                        'Time' : str(dt.datetime.now())}, ignore_index = True)
                ...
                if len(self.data) < self.averaging_period - 1:
                    ...
                    self.min_length = self.averaging_period - 1
                    print('len(self.data) is less than self.min_length (self.averaging_period - 1), so self.min_length is set as (self.averaging_period - 1).')
                    self.logger_monitor('len(self.data) is less than self.min_length (self.averaging_period - 1), so self.min_length is set as (self.averaging_period - 1).')
                else:
                    self.min_length = len(self.data)
                    print('len(self.data) hits self.min_length (self.averaging_period - 1) and above, self.min_length is set as len(self.data).')
                    self.logger_monitor('len(self.data) hits self.min_length (self.averaging_period - 1) and above, self.min_length is set as len(self.data).')
        ...
        if dt.datetime.now().day == 28 and dt.datetime.now().hour == 12 and dt.datetime.now().minute == 1:
            if self.time_flag_unload:
                print('\n' + 96 * '=' + '\n')
                self.logger_monitor('\n' + 96 * '=' + '\n')
                self.model_persisted.save('trading_model_usd_jpy.h5')
        ...
        if dt.datetime.now().day >= 1 and dt.datetime.now().day <= 8 and dt.datetime.today().weekday() in [0] and dt.datetime.now().hour == 4 and dt.datetime.now().minute == 45:
            ...
            if self.load_reoptimized_model_time_flag:
                ...
                try:
                    ...
                    print('\n' + 96 * '=' + '\n')
                    self.logger_monitor('\n' + 96 * '=' + '\n')
                    self.model_persisted = load_model('reoptimized_model_usd_jpy.h5')
                    ...
                    print('\n' + 96 * '=' + '\n')
                    self.logger_monitor('\n' + 96 * '=' + '\n')
                    self.pkl_file_units = open('units_transfer_usd_jpy.pkl', 'rb')
                    ...
                    print('\n' + 96 * '=' + '\n')
                    self.logger_monitor('\n' + 96 * '=' + '\n')
                    self.pkl_file_vAr = open('vAr_transfer_usd_jpy.pkl', 'rb')
                    ...
                    print('\n' + 96 * '=' + '\n')
                    self.logger_monitor('\n' + 96 * '=' + '\n')
                    self.pkl_file_df_test = open('df_test_transfer_usd_jpy.pkl', 'rb')
                    self.df_test = pickle.load(self.pkl_file_df_test)
                    self.pkl_file_df_test.close()
                    print('df_test_transfer_usd_jpy is loaded from SSD. Use jupyter notebook to check if needed.')
                    self.logger_monitor('df_test_transfer_usd_jpy is loaded from SSD. Use jupyter notebook to check if needed.')
                except:
...
if __name__ == '__main__':
    ...
    model_persisted = load_model('trading_model_usd_jpy.h5')
    print(f'model_persisted is initialized with trading_model_usd_jpy.h5 from SSD @ {dt.datetime.now()}')
    
    pkl_file_vAr_initial_setup = open('vAr_initial_setup_usd_jpy.pkl', 'rb')
    vAr = pickle.load(pkl_file_vAr_initial_setup)
    pkl_file_vAr_initial_setup.close()
    print(f'vAr is initialized with vAr_initial_setup_usd_jpy.pkl from SSD @ {dt.datetime.now()}')
    
    pkl_file_units_initial_setup = open('units_initial_setup_usd_jpy.pkl', 'rb')
    units = pickle.load(pkl_file_units_initial_setup)
    pkl_file_units_initial_setup.close()
    
    pkl_file_df_test = open('df_test_setup_usd_jpy.pkl', 'rb')
